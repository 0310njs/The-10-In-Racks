...
@test "all documented parameters exist in SCAD file" {
    local params=(
        ...
        component1_width component1_depth component1_height
        ...
        keystones1 keystones2 guide_rails_on
    )
    for p in "${params[@]}"; do
...
@test "unknown parameter name is caught before rendering" {
    run render_views "should_not_exist" -D 'component_height=30'
    [ "$status" -ne 0 ]
}
...
@test "default" {
    render_views "default" -D  'guide_rails_on=false'
    assert_views_exist "default"
}

@test "6-inch rack" {
    render_views "6inch" -D 'rack_width=152.4' -D  'guide_rails_on=false'
    assert_views_exist "6inch"
}

@test "2U" {
    render_views "2u" -D 'rack_height=2' -D  'guide_rails_on=false'
    assert_views_exist "2u"
}

@test "0.5U" {
    render_views "half_u" -D 'rack_height=0.5' -D 'component1_height=15' -D  'guide_rails_on=false'
    assert_views_exist "half_u"
}

@test "solid front plate" {
    render_views "solid_front" -D 'front_plate_hole=false' -D  'guide_rails_on=false'
    assert_views_exist "solid_front"
}

@test "air holes disabled" {
    render_views "no_air" -D 'component1_air_holes=false' -D  'guide_rails_on=false'
...
@test "thick case walls" {
    render_views "thick_walls" -D 'case_thickness=10' -D  'guide_rails_on=false'
    assert_views_exist "thick_walls"
}

@test "keystone jacks" {
    render_views "keystones" -D 'keystones1=true' -D 'keystones2=true' -D  'guide_rails_on=false'
    assert_views_exist "keystones"
}

@test "keystone jacks disabled" {
    render_views "no_keystones" -D 'keystones1=false' -D 'keystones2=false' -D  'guide_rails_on=false'
...
@test "UniFi Cloud Key G2+ (131.2x27.1x134.2) 4U" {
    render_views "unifi_cloud_key_g2plus" \
        -D 'component1_width=131.2' -D 'component1_depth=27.1' -D 'component1_height=134.2' \
        -D 'rack_height=4' -D  'guide_rails_on=false'
...
@test "UniFi Lite 16 POE (192x185x44)" {
    render_views "unifi_lite_16_poe" \
        -D 'component1_width=192' -D 'component1_depth=185' -D 'component1_height=44'\
	-D 'rack_height=1.5' -D  'guide_rails_on=false'
...
@test "M4 Mac Mini (127x127x50) 2U" {
    render_views "m4_mac_mini" \
        -D 'component1_width=127' -D 'component1_depth=127' -D 'component1_height=50' \
        -D 'rack_height=2' -D  'guide_rails_on=false'
    assert_views_exist "m4_mac_mini"
}

@test "BeeLink ME Mini (99x99x99) 3U" {
    render_views "beelink_me_mini" \
        -D 'component1_width=99' -D 'component1_depth=99' -D 'component1_height=99' \
        -D 'rack_height=3' -D  'guide_rails_on=false'
...
@test "Synology DS223j (165x100x225.5) 5U" {
    render_views "synology_ds223j" \
        -D 'component1_width=165' -D 'component1_depth=100' -D 'component1_height=225.5' \
        -D 'rack_height=5.5' -D  'guide_rails_on=false'
    assert_views_exist "synology_ds223j"
}

@test "Synology DS223 (165x108x232.7) 5U" {
    render_views "synology_ds223" \
        -D 'component1_width=165' -D 'component1_depth=108' -D 'component1_height=232.7' \
        -D 'rack_height=5.5' -D  'guide_rails_on=false'
...
@test "Synology DS124 (166x71x224) 5U" {
    render_views "synology_ds124" \
        -D 'component1_width=166' -D 'component1_depth=71' -D 'component1_height=224' \
        -D 'rack_height=5.5' -D  'guide_rails_on=false'
